  ...
  **New attachment source (read this too)**: the agent can now also capture an image the **AI itself generates** in its reply (DALL-E/Imagen-style), not just images the user uploads. It shows up in `attachments` exactly like a clipboard/file-dialog image, with a new `source: "ai_generated"` value — see Page 2's Record shape A below for the full detail, including a real fidelity caveat (it's a screenshot, not the model's original bytes) and an **action item**: if your ingest endpoint validates `attachments[].source` against a strict enum, it needs `ai_generated` added or these records will get silently rejected.
  ...
  **The old "store a full base URL per device" model, and the ngrok-URL-changes-on-every-restart pain that came with it, are both gone.** The device needs no public inbound endpoint at all anymore — delete that requirement from your setup notes entirely, not just the registry schema.
  ...
  ```json
  {
    ...
    "attachments": [
      ...
      {
        "mime_type": "image/png",
        "filename": "ai_generated_image_0.png",
        "source": "ai_generated",
        "size_bytes": 512044,
        "sha256": "...",
        "data_base64": "iVBORw0KGgoAAAANSUhEUgAA..."
  ...
  **`attachments`** — images alongside this turn, from three possible sources. Always present as a field; an empty array `[]` for an ordinary text-only turn. `source` is one of:
  - `"clipboard"` — the user pasted an image.
  - `"file_dialog"` — the user attached one via the native file-picker dialog.
  - `"ai_generated"` — **new**: the AI itself generated this image in its reply (DALL-E/Imagen-style). Captured as a **screen-region screenshot** of the rendered image, not the model's original file — a real fidelity difference worth surfacing in the UI (e.g. a distinct badge/caption, not just treating it identically to a user's own upload), since it's a re-encoded PNG of what was on screen, not the source bytes. **Action item for your ingest endpoint**: if `attachments[].source` is validated against a strict enum server-side, add `ai_generated` to it, or these records will be silently rejected at ingest.

  `data_base64` is the raw image bytes, base64-encoded — decode and render as an `<img>`/`st.image` from that directly, no separate fetch needed, regardless of source. **A turn can have `prompt == ""` with a non-empty `attachments` list** — that's a real, valid case: the user sent an image with no caption at all. Don't treat an empty prompt as "nothing to show" for these rows (an `ai_generated`-only attachment on a turn that DID have a real prompt is also normal and expected — the user asked for an image, the AI's whole "response" text may be minimal or empty). **Not captured**: drag-and-drop attachments — see the agent repo's `docs/KT_GUIDE.md` for why (investigated, not a simple gap).
  ...
  **Known gap**: this shape does not carry a clean `provider` field — only `source_app` (a raw process name). Possible follow-up if a provider tag is wanted here too.
  ...
  - **Attachments**: if `attachments` is non-empty, render each one as a thumbnail (`st.image(base64.b64decode(att["data_base64"]))`) inside the row/expander, with a small caption showing `filename` and `source`. Handle the image-only case explicitly in the layout — an empty `prompt` next to a rendered thumbnail is the normal, expected look for that row, not a missing-data bug. For `source: "ai_generated"`, show a distinct label/badge (e.g. "AI-generated") rather than captioning it the same as a user's own clipboard/file-dialog upload — it's meaningfully different content (the AI's own output vs. something the user attached) and a re-encoded screenshot rather than the original bytes, both worth being visually upfront about.
  ...
  10. **Confirm AI-generated image capture** — on the test device, ask a monitored platform to generate an image (e.g. "generate an image of a cat" on ChatGPT/Gemini), and confirm the resulting record's `attachments` array has an entry with `source: "ai_generated"` that renders correctly. If your ingest endpoint enum-validates `source`, this is exactly the case that would get silently rejected if `ai_generated` wasn't added to it — an empty `attachments` array here despite a generated image being visible on screen is the tell.
  11. **Confirm reconnection** — kill the test device's network briefly (or restart the agent), confirm it reconnects with visible backoff in `agent.log`, and that a `publish` sent while it was disconnected either gets picked up via the `hello`-hash drift check on reconnect, or is at least clearly visible as "device was offline" in the portal rather than silently lost.
  ...
  6. Activity Logs page: table/feed reading from the ingest store, tagged and sorted per the rules above, with an autorefresh or manual-refresh mechanism, rendering `attachments` thumbnails per row (including the image-only, empty-prompt case, and a distinct badge/caption for `source: "ai_generated"` vs. the two user-attached sources).
  7. **If `attachments[].source` is enum-validated at ingest, add `"ai_generated"` to that enum** — otherwise records containing an AI-generated image get silently rejected. This is the one action item in this whole doc that's a hard requirement, not a nice-to-have, since the agent already ships this source value today.
  8. Flag back to the agent side if a device `status` message type is wanted, and/or if detection-alert rows (shape B) need a real provider tag.
  9. Storage note for `attachments`: base64 image blobs are meaningfully bigger than anything this store has held so far (up to ~11MB per image after base64 inflation of the agent's 8MB cap) — if using a CSV for the ingest store, this will get unwieldy fast; SQLite (already suggested as an option) handles it fine. Decide before volume makes it a problem, not after.
